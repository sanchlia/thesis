...
class UtilityParity(ClassificationMoment):
    ...
    def default_objective(self):
        ...
        # print(f'init default {self.tags.shape}=')
        return ErrorRate(tags = self.tags)
    ...
    def load_data(
        ...
        *,
        ...
        if fair_weights is None:
            ...
            positive_df = pd.DataFrame({"label": y})
            positive_df["Yf"] = 1
            positive_df["weights"] = fair_weights

            negative_df = pd.DataFrame({"label": y})
            negative_df["Yf"] = 0
            negative_df["weights"] = 1 - fair_weights

            combined_weights = np.concatenate((positive_df["weights"], negative_df["weights"]))
            x_c = np.copy(X)
            total_x = np.concatenate((X, x_c))
            total_sv = np.concatenate((sensitive_features, sensitive_features))
            total_y = np.concatenate((positive_df["Yf"], negative_df["Yf"]))
            # print('UP100: just here')
            _, y_train, sf_train, cf_train = _validate_and_reformat_input(
                total_x,
                total_y,
                enforce_binary_labels=True,
                sensitive_features=total_sv,
                control_features=None,
            )
            # print('UP108: just here')

            base_event = y_train.apply(lambda v: _LABEL + "=" + str(v))

            event = _merge_event_and_control_columns(base_event, cf_train)
            temp = pd.DataFrame({_LABEL: total_y})
            temp[_GROUP_ID] = total_sv
            temp["weights"] =  combined_weights
            temp[_EVENT] = event
            self.prob_event = temp.groupby([_EVENT])["weights"].sum()/(self.total_samples * 0.5)
            self.prob_group_event = (
                temp.groupby([_EVENT, _GROUP_ID])["weights"].sum()/(self.total_samples * 0.5)
            )
            # E[P(Y = {0,1}| S,X, Y')]
            #self.prob_event = temp.groupby([_EVENT])["weights"].sum()/(self.total_samples)
            # self.prob_group_event = (
            #     temp.groupby([_EVENT, _GROUP_ID])["weights"].sum() / (self.total_samples)
            # )
            #  TODO: Commented this for continuity in NeurIPS paper. Revert for TPM. 
            temp["weights"] = combined_weights
            self.fair_df = temp
            self.fn_cost = positive_df
            self.fp_cost = negative_df
            self.X = total_x
            self._y = total_y
            # self.total_samples = temp.shape[0]
            self.tags = temp
            # print(f'{self.tags.shape}=')
            
            utilities = np.vstack(
                [
                    np.zeros(total_y.shape, dtype=np.float64),
                    np.ones(total_y.shape, dtype=np.float64),
                ]
            ).T
            self.utilities = utilities
    ...
    def gamma(self, predictor, tags = None):
        ...
        # reference
        # self.prob_event = temp.groupby([_EVENT])["weights"].sum()/(2*self.total_samples)
        # self.prob_group_event = (
        #     temp.groupby([_EVENT, _GROUP_ID])["weights"].sum() / (2*self.total_samples)
        # )

        # self.tags["wp"] = self.tags["weights"]
        ...
        # print(f'what is {expect_event=}')
        # print(f'what is {expect_group_event=}')
        # expect_group_event[_UPPER_BOUND_DIFF] = (
        #     self.ratio * (expect_group_event["weights"]) - (expect_event["weights"])
        # )
        # expect_group_event[_LOWER_BOUND_DIFF] = (
        #     -(expect_group_event["weights"] ) + self.ratio * (expect_event["weights"])
        # )
...
class EqualizedOdds(UtilityParity):
    ...
    def load_data(self, X, y, *, sensitive_features, fair_weights, control_features=None):
        ...
        base_event = y_train.apply(lambda v: _LABEL + "=" + str(v))
        event = _merge_event_and_control_columns(base_event, cf_train)
        super().load_data(X, y_train, event=event, sensitive_features=sf_train, fair_weights = fair_weights)
