...
class _Lagrangian:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # print("in langrangian 2")
        # print(kwargs)

        self.constraints = constraints
        # print(self.constraints)
        self.constraints.load_data(X, y, **kwargs)
        # print("load_data occured")
        if objective is None:
            # print("confirm that objective is none")
    ...
    def _eval(self, Q, lambda_vec):
        ...
        if callable(Q):
            ...
            # print("128: probably enters here")
    ...
    def _call_oracle(self, lambda_vec):
        try:
            signed_weights = self.obj.signed_weights(fair_df = self.constraints.fair_df, tags = self.constraints.tags) + self.constraints.signed_weights(
                lambda_vec
            )
        ...
        # print(f'{self.constraints.total_samples=}')
    ...
    def best_h(self, lambda_vec):
        ...
        h_error = self.obj.gamma(h, X = self.constraints.X, tags = self.constraints.tags).iloc[0]
        h_gamma = self.constraints.gamma(h, tags = self.constraints.tags)
        h_value = h_error + h_gamma.dot(lambda_vec)
        # print(f'{h_gamma.dot(lambda_vec)=}')
        print(f'{h_value}')
        if not self.hs.empty:
            # print(f'is it empty ever?"')
