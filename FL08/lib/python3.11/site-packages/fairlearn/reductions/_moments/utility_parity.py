...
class UtilityParity(ClassificationMoment):
    ...
    def _get_costs(self):
        fp_cost = np.where(self.tags[_LABEL] == 0, self.tags["weights"], 1 - self.tags["weights"])
        fn_cost = np.where(self.tags[_LABEL] == 1, self.tags["weights"], 1 - self.tags["weights"])

        # fp_cost =  1 - self.tags["weights"]
        # fn_cost = self.tags["weights"]
        return fp_cost, fn_cost
    
    def default_objective(self):
        """Return the default objective for moments of this kind."""
        if self._fair_weights is not None:
            fp_cost, fn_cost = self._get_costs()
            cost = {"fn":fn_cost, "fp": fp_cost}
            return ErrorRate(costs = cost)
        return ErrorRate()
    ...
    def load_data(
        ...
        *,
        ...
        """
        super().load_data(X, y, sensitive_features=sensitive_features, fair_weights = fair_weights)
        ...
        if self._fair_weights is not None:
            utilities = np.vstack(
                [
                    np.zeros(self._y.shape, dtype=np.float64),
                    np.ones(self._y.shape, dtype=np.float64),
                ]
            ).T
            self.utilities = utilities
            # P(Y = 0) is 
            # new logic
            self.prob_event = self.tags.groupby([_EVENT])["weights"].sum()/(self.total_samples)
            # self.prob_event = self.p_y
            ...
            # self.prob_group_event = pd.Series([self.p_y['label=0']*0.5, self.p_y['label=0']*0.5 , self.p_y['label=1']*0.5, self.p_y['label=1']*0.5], index =group_event_index)
            self.prob_group_event = (
                self.tags.groupby([_EVENT, _GROUP_ID])["weights"].sum()/(self.total_samples)
            )
            # OG doubling of space with weights assumed to be P(Y=y|X,S,Y_obs)
            # self.prob_event = self.tags.groupby([_EVENT])["weights"].sum()/(self.total_samples//2)
            # self.prob_group_event = (
            #     self.tags.groupby([_EVENT, _GROUP_ID])["weights"].sum()/(self.total_samples//2)
            # )
        else:
            self.tags['']
    ...
    def estimate_conditional(self):
        ...
        # y is available, P(Y=1|E) and P(Y=0|E) is available. 
        # calculate P(Y = y | S = s)
        # df.groupby()
        temp_tags = self.tags.copy()
        temp_tags['positive_weights'] = np.where(temp_tags['label'] == 1 , temp_tags['weights'], 1 - temp_tags['weights'])
        temp_tags['negative_weights'] = np.where(temp_tags['label'] == 0 , temp_tags['weights'], 1 - temp_tags['weights'])
        # E(F(x) | Y^)
        # e_y_1 = sum(temp_tags['positive_weights'] * temp_tags[_PREDICTION])/ ( (len(temp_tags)) * self.p_y['label=1'])
        # e_y_0 = sum(temp_tags['negative_weights'] * temp_tags[_PREDICTION])/ ( (len(temp_tags)) * self.p_y['label=0'])
        # e_y_1 = sum(temp_tags['positive_weights'] * temp_tags[_PREDICTION])/ ( (len(temp_tags)) * self.prob_event['label=1'])
        # e_y_0 = sum(temp_tags['negative_weights'] * temp_tags[_PREDICTION])/ ( (len(temp_tags)) * self.prob_event['label=0'])
        expect_event = pd.Series([e_y_0, e_y_1], index = ['label=0', 'label=1'])
        expect_event.index.names = ['event']
        # E(F(x) | S, Y )
        group_event_tuples = [('label=0', 0), ('label=0', 1) , ('label=1', 0), ('label=1', 1)]
        group_event_index = pd.MultiIndex.from_tuples(group_event_tuples, names = ['event', 'group_id'])
        n_s = self.tags.groupby('group_id').size()

        e_s_0_y_1 = (temp_tags[temp_tags['group_id'] == 0]['positive_weights'] * temp_tags[_PREDICTION]).sum()/ ( n_s[0] * self.p_y_s.loc[('label=1',0)])
        e_s_0_y_0 = (temp_tags[temp_tags['group_id'] == 0]['negative_weights'] * temp_tags[_PREDICTION]).sum()/ ( n_s[0] * self.p_y_s.loc[('label=0',0)])
        e_s_1_y_1 = (temp_tags[temp_tags['group_id'] == 1]['positive_weights'] * temp_tags[_PREDICTION]).sum()/ ( n_s[1] * self.p_y_s.loc[('label=1',1)])
        e_s_1_y_0 = (temp_tags[temp_tags['group_id'] == 1]['negative_weights'] * temp_tags[_PREDICTION]).sum()/ ( n_s[1] * self.p_y_s.loc[('label=0',1)])
        expect_group_event = pd.Series([e_s_0_y_0, e_s_1_y_0, e_s_0_y_1,  e_s_1_y_1], index = group_event_index)
        return expect_event, expect_group_event
    ...
    def gamma(self, predictor):
        ...
        if self.p_y is not None and self.p_y_s is not None:
            # event index 
            # event_index_tuple = [('label=0'), ('label=1')]
            # event_index = pd.MultiIndex.from_tuples(event_index_tuple, names = ['event'])
            ...
            # event =  self.tags[self.tags['pred'] == 1].groupby(_EVENT)['weights'].sum()
            # expect_event = event/(self.p_y * ( len(self.tags) * 0.5))
            # # calculate n_s
            # expect_group_event =  self.tags[self.tags['pred'] == 1].groupby([_EVENT, _GROUP_ID]).sum()
            # # expect_group_event = expect_group_event
            # n_s = self.tags.groupby('group_id').size()/2
            # expect_group_event = expect_group_event['weights']/(n_s * self.p_y_s)
            expect_event, expect_group_event = self.estimate_conditional()
            upper_bound_diff = self.ratio * expect_group_event - expect_event
            
            lower_bound_diff = -expect_group_event + self.ratio * expect_event
            g_signed = pd.concat(
            [
                upper_bound_diff,
                lower_bound_diff,
...
class EqualizedOdds(UtilityParity):
    ...
    #     return _fair_weights, positive_df, negative_df
    ...
    def load_data(self, X, y, *, sensitive_features, fair_weights, control_features=None):
        """Load the specified data into the object."""
        if fair_weights is not None:
            self.p_y = fair_weights['p_y']
            self.p_y_s = fair_weights['p_y_s']
        ...
        event = _merge_event_and_control_columns(base_event, cf_train)
        super().load_data(X, y_train, event=event, sensitive_features=sf_train, fair_weights = self._fair_weights)
