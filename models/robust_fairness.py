...
class RobustFairness():
    def __init__(self, random_initialization=False, logging=False):
        self.h = eopp_fair_covariate_shift_logloss(
            verbose=1 if logging else 0, tol=1e-7, random_initialization=False, trg_grg_marginal_matching_covariate_shift=True
        )
        h.trg_grp_marginal_matching = True

    def fit(self, X_src, Y_src, S_src, X_trg, S_trg, mu_range=[-1.5,1.5], C=0.001, max_epoch=3, max_iter=3000, random_start=True):
        h.C = C
        h.max_epoch = max_epoch
        h.max_iter = max_iter
        h.random_start = random_start
        h.fit(
            X_src,
            Y_src,
            S_src,
    ...
    def predict(self, X, A):
        return h.predict(X,A,np.ones_like(A))

    def predict_proba(self, X, A):
        return h.predict_proba(X,A,np.ones_like(A))
